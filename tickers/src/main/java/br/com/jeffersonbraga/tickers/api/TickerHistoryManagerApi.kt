...
@RestController
@RequestMapping("ticker/manager/history")
class TickerHistoryManagerApi {
    ...
    @GetMapping
    private fun tickerHistoryManager() {
        ...
        listaCompras.distinctBy { it.ticker }.forEach { listSearch.add(it.ticker!!) }
        var stockList: Map<String, Stock>? = null
        try {

            //var stockList = YahooFinance.get(listSearch.toTypedArray(), true)
            var calendar = Calendar.getInstance()
            calendar.set(Calendar.MONTH, -15)
            var stockList = YahooFinance.get(listSearch.toTypedArray(), calendar, Interval.DAILY)
