...
class TickerManagerApi {
    ...
    private fun tickerManager() {
        ...
        try {
            ...
            var stockList = YahooFinance.get(listSearch, false)
                ...
                try {
                    for (itHistory in stock.history) {
                        ...
                        if (itHistory.close != null) {
                            tickerHistory.close = itHistory.close.toLong()
                        } else {
                            tickerHistory.close = 0L
                        }
                        if (itHistory.low != null) {
                            tickerHistory.low = itHistory.low.toLong()
                        } else {
                            tickerHistory.low = 0L
                        }
                        if (itHistory.high != null) {
                            tickerHistory.high = itHistory.high.toLong()
                        } else {
                            tickerHistory.high = 0L
                        }
                        if (itHistory.open != null) {
                            tickerHistory.open = itHistory.open.toLong()
                        } else {
                            tickerHistory.open = 0L
                        }
                        if (itHistory.volume != null) {
                            tickerHistory.volume = itHistory.volume.toLong()
                        } else {
                            tickerHistory.volume = 0L
                        }
