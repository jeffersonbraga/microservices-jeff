...
class TickerManagerApi {
    ...
    private fun tickerManager() {
        ...
        var stockList: Map<String, Stock>? = null
        val listaCompras = buyRepository.findAll()
        try {

            var listaString = arrayOf ("")
            listaCompras.forEach{ listaString = listaString.plusElement(it.ticker!!) }
            var calendar = Calendar.getInstance()
            calendar.set(Calendar.MONTH, -12)
            var stockList = YahooFinance.get(listaString, calendar, Interval.DAILY)
            stockList.entries.forEach {
                ...
                try {
                    ...
                    val tickerCurrentStatus = TickerCurrentStatus()
                    tickerCurrentStatus.ticker = ticker.ticker
                    tickerCurrentStatus.valorAtual = price
                    tickerCurrentStatus.fechamentoAnterior = previousClose
                    tickerCurrentStatus.mudanca = change
                    tickerCurrentStatus.avg50 = avg50
                    tickerCurrentStatus.avg200 = avg200
